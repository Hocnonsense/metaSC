 ...
 * @LastEditors: Hwrn
 * @LastEditTime: 2020-10-05 15:38:04
 * @FilePath: /HScripts/Python/mylib/figure/color.py
...
def rgb2hsl_matrix(colors):
    ...
    np.seterr(divide='ignore', invalid='ignore')
    ss = np.where(ls < 0.5, del_Max/(var_Max+var_Min),
...
def img_color(img_name):
    ...
    im_g = Image.fromarray(np.uint8(narray))
    plt.imshow(im_g)
    plt.show()
    im_g.save(img_name)
