 ...
 * @LastEditors: Hwrn
 * @LastEditTime: 2022-07-10 12:52:05
 * @FilePath: /metaSC/PyLib/figure/transhexmesh.py
...
def draw_transparent_hexagonal_mesh(row, column, unit_length):
    ...
    imgarray = np.zeros(
        shape=(
            int(col * 3 / 2 * collen + collen / 2),
            int(row * rowlen * 2 + rowlen) + width,
            4,
        )
    )
    for j in range(col // 2):
        ...
        imgarray[
            Aj:Bj, int(row * 2 * rowlen) : int(row * 2 * rowlen) + width, :
        ] = color  # last a|b
        imgarray[
            Cj:Dj,
            int(row * 2 * rowlen + rowlen) : int(row * 2 * rowlen + rowlen) + width,
            :,
        ] = color  # last c|d
        for x in range(Dj, int(_j + collen * 7 / 2)):
            y1 = int(row * 2 * rowlen + rowlen - (x - Dj) / (Aj - dj) * (CDi - ABi))  # type: ignore
            imgarray[x, y1 : y1 + width, :] = color  # last A/d
        for x in range(Bj, Cj):
            y3 = int(row * 2 * rowlen + (x - Bj) / (Cj - Bj) * (CDi - ABi))  # type: ignore
