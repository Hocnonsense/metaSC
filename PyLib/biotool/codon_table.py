...
def weighted_by_abundance(gene_95_results_handle, abundance_table, metadata_handle, gene_cluster_map_handle='NA', output_flag='NA'):
    """
    ...
    new_meta_index = ['HOT229_0025m' if x=='HOT229_1_0025m' else x for x in metadata.index.values]
    new_meta_index = ['HOT237_0025m' if x=='HOT237_1_0025m' else x for x in new_meta_index]
    new_meta_index = ['HOT234_0200m' if x=='HOT234_1_0200m' else x for x in new_meta_index]
    new_meta_index = ['HOT229_0500m' if x=='HOT229_1_0500m' else x for x in new_meta_index]
    new_meta_index = ['HOT232_0500m' if x=='HOT232_1_0500m' else x for x in new_meta_index]
    new_meta_index = ['HOT237_0500m' if x=='HOT237_1_0500m' else x for x in new_meta_index]
    new_meta_index = ['HOT236_0500m' if x=='HOT236_1_0500m' else x for x in new_meta_index]
    new_meta_index = ['HOT234_0200m' if x=='HOT234_1_0200m' else x for x in new_meta_index]
    new_meta_index = ['HOT237_0770m' if x=='HOT237_1_0770m' else x for x in new_meta_index]

    metadata.index = new_meta_index
    # translate cluster handles
    if gene_cluster_map_handle != 'NA':
        mOTU_seq_to_clusters=pd.read_csv(gene_cluster_map_handle, index_col=0, sep='\t', names=['clusters'] )
    ...
    while line:
            ...
            if gene_cluster_map_handle != 'NA':
                cluster_sequence = mOTU_seq_to_clusters.loc[line_values['gene.id'], 'clusters']
            else:
                ...
                for sample in sample_names:
                    ...
                    for key in values_to_average:
                        if line_values[key] != 'Nan' and line_values[key] != 'None':
                            tally_dictionary[sample][key]['sum']+= float(line_values[key]) * abundance
    ...
    # calculate averages
    final_dictionary = dict([(sample, {x:'NA' for x in values_to_average}) for sample in sample_names])

    for sample in tally_dictionary.keys():
        for value in values_to_average:
            if tally_dictionary[sample][value]['count'] > 0:
                final_dictionary[sample][value] = tally_dictionary[sample][value]['sum'] / tally_dictionary[sample][value]['count']
            final_dictionary[sample]['N_ARSC_count']= tally_dictionary[sample][value]['sum']
