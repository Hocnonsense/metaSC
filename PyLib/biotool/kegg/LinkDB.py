 ...
 * @LastEditors: Hwrn
 * @LastEditTime: 2022-02-08 20:48:25
 * @FilePath: /metaSC/PyLib/biotool/kegg/LinkDB.py
 * @Description:
"""

import re
from typing import Dict, Generator, List, Tuple, Union, TextIO
...
def path2tsv_iter(
    text: TextIO,
) -> Generator[Tuple[str, List[str], str, List[str]], None, None]:
    """
    ...
    * @return {*}
    ...
    for line in text:
        if line.startswith("K") and set(line[1:6]).issubset(set("1234567890")):
            ...
            genes_ = genes.split(", ")
            values_ = values.strip()
            desc, EC = (values_.split("[EC:", 1) + [""])[:2]
            desc_: str = desc.strip()
            EC_ = EC.split("]", 1)[0].split()
            yield ID, genes_, desc_, EC_


def module_from_brite(
    source: Union[str, TextIO], brite_path: str = "", module_path: str = ""
) -> Tuple[List[Tuple[str, str, str, str, str]], List[Tuple[str, KModule]]]:
...
def map_KO_dict(KO_str: str, ko_match: Dict[str, float]) -> str:
    ...
    for match in re.finditer(KO_PATTERN, KO_str):
        ...
        if KO in ko_match:
            ...
            KO_value_ = "{value:.{len_value}f}".format(
                value=KO_value, len_value=5 - len(str(KO_value // 1).split(".")[0])
            )
        else:
            KO_value_ = " NaN. "
        KO_str = KO_str.replace(KO, KO_value_)
    return KO_str
