...
def get_gene_tpg(countfile, subsets
    ...
    with open(countfile) as file_in:
        ...
        for values in featureCounts_iter(file_in):
            ...
            gene_tpg[gene] = [readCount / length for readCount in readCounts]

    logger.info(f"total {len(samples)} bam")
    return samples, gene_tpg
...
def set_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument('--tpg', '--featuer-counts', default='', type=str,
